    ...
    #region Public Variables
        ...
        private void OnInitializeCallback() {
            ...
            if (FB.IsInitialized)
            {
                ...
                {
                    APSdkLogger.Log(string.Format("AudienceNetwork = {0}", _isATTEnabled));
                    AudienceNetwork.AdSettings.SetAdvertiserTrackingEnabled(_isATTEnabled);
                }
                else
                    APSdkLogger.LogWarning(string.Format("AudienceNetwork.AdSettings.SetAdvertiserTrackingEnabled() -> is not set as non iOS platform"));
